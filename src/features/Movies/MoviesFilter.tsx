...
export default function MoviesFilter({ onApply }: MoviesFilterProps) {
  ...
  const debounceFetchKeywordsOptions = useMemo(
    () =>
  ...
  return (
    <Paper sx={{ m: 2, p: 0.5 }}>
      <form onSubmit={handleSubmit(onApply)}>
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <Autocomplete
                ...
                onInputChange={(_, value) => debounceFetchKeywordsOptions(value)}
              />
            )}
          />
        </FormControl>
        ...
        <Button type="submit" variant="contained" startIcon={<FilterAltOutlined />} sx={{ m: 2 }} disabled={!formState.isDirty}>
          Apply filter
        </Button>
      </form>
    </Paper>
